...
impl ChunkType {
    fn bytes(&self) -> [u8; 4] {
        self.chunk_type[..4].try_into().unwrap()
    }
    ...
    fn is_safe_to_copy(&self) -> bool {
        if self.chunk_type[3].is_ascii_uppercase() {
            return false;
        } else {
            return true;
        }
...
impl Display for ChunkType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s: String = self.chunk_type.iter().map(|&byte| byte as char).collect();
        write!(f, "{}", s)
    }
}
...
impl TryFrom<[u8; 4]> for ChunkType {
    ...
    fn try_from(value: [u8; 4]) -> Result<Self, Self::Error> {
        for c in value.iter() {
            if !c.is_ascii() {
                return Err("Value is not ascii".into());
            }
        }
        Ok(Self {
